...
def test_parses_fixture_metadata() -> None:
    ...
    assert post.metadata.subtitle == "Our God is a devourer, who makes things only for the swallowing."
    assert post.metadata.author == "Sam Kriss"
    assert post.metadata.date == "2022-09-18"
    assert post.metadata.canonical_url == "https://samkriss.substack.com/p/the-internet-is-already-over"
    assert post.metadata.hero_image and post.metadata.hero_image.startswith("https://substackcdn.com/")
...
def test_renders_substack_footnotes_as_pandoc_footnotes() -> None:
    ...
    assert "[^1]: I am *very disappointed* that this scene never appears in Disney’s *Moana*." in markdown
